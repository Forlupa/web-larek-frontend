...
#### Класс Product 
Класс, хранящий в себе данные о товаре. Наследуется от абстрактного класса Model.\
В полях хранятся даные карточки. 

#### Класс AppState 
Класс отвечает за хранение и логику работы с данными приложения.\
В полях класса хранятся следующие данные:
-  `basket: Product[]` - корзина с товарами
-  `cards: Product[]` - массив товаров
...
-  `setOrderForm` - сеттер который заполняет поля формы
-  `setItems` - сеттер который передаёт выбранные товары в заказ
-  `setCatalog` - сеттер который создаёт каталог товаров
...
- `catalog` - сеттер каталога товаров
...
#### Класс CatalogItem 
Класс, описывающий карточку товара в каталоге. Наследуется от класса Card.

Конструктор: 
- `constructor(container: HTMLElement, actions?: ICardActions)` - принимает родительский контейнер и объект с колбэк функциями


#### Класс CatalogItemPreview 
Класс, описывающий превью карточки товара. Наследуется от класса Card.
...
- `updateIndexes` - метод обновляющий индексы товаров в корзине после удаления
- `disableButton` - метод отключающий кнопку "Оформить"

#### Класс BasketItem
Класс, описывающий товар в корзине. Наследуется от абстрактного класса Component.

Конструктор: 
- `constructor(protected blockName: string, container: HTMLElement, actions?: IBasketItemActions)` - принимает имя блока, родительский элемент и объект с колбэк функциями

В полях класса хранятся ссылки на внутренние элементы.\
Класс имеет такие свойства и методы:
- `title` - сеттер названия товара
- `index` - сеттер индекса товара
- `price` - сеттер цены товара 
...
- `valid` - ограничивает доступ к кнопке при ошибках валидации
...
В полях класса хранятся ссылки на внутренние элементы.\
Класс имеет такие методы:
- `bill` - сеттер для надписи о том, сколько синапсов было списано со счета
